...
async function verify() {
  ...
  console.log('Triggering video processing (non-blocking)...');
  fetch('http://localhost:5000/api/videos/process', {
    method: 'POST',
    headers: {
  ...
  let attempts = 0;
  const maxAttempts = 150; // 150 * 3 seconds = 450 seconds (7.5 minutes) maximum wait
  ...
  if (attempts >= maxAttempts) {
    throw new Error('Verification timed out after 7.5 minutes.');
  }
